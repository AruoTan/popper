...
const snapshot: DictionarySnapshot = {
  ...
  suggestions: [{ word: "account for", explanation: "解释" }],
  error: null,
...
const props = () => ({
  snapshot,
  onAi: vi.fn().mockResolvedValue(true),
  onQuery: vi.fn().mockResolvedValue(undefined),
});
beforeEach(() => {
  window._popper_ = {
    queryDictionary: vi.fn().mockResolvedValue("request"),
    suggestDictionary: vi.fn().mockResolvedValue([]),
    cancelDictionaryInput: vi.fn().mockResolvedValue(undefined),
    getEudicBooks: vi
      .fn()
      .mockResolvedValue([{ id: "123456789012345678", name: "学习", language: "en" }]),
...
describe("dictionary panel", () => {
  ...
  it("queries the selected suggestion, not the first candidate automatically", async () => {
    const input = props();
    render(<DictionaryPanel {...input} />);
    expect(window._popper_.queryDictionary).not.toHaveBeenCalled();
  ...
  it("has no separate input and places actions beside phonetics and suggestions last", () => {
    ...
    expect(container.querySelector(".dictionary-panel")!.lastElementChild).toHaveClass(
      "dictionary-suggestions",
    );
  });
  it("requires explicit book selection and prevents duplicate submission", async () => {
    const pending = deferred<void>();
    vi.mocked(window._popper_.addEudicWord!).mockReturnValue(pending.promise);
    ...
    expect(window._popper_.addEudicWord).toHaveBeenCalledExactlyOnceWith(
      "session",
      1,
      "123456789012345678",
    );
  ...
  it("keeps book selection after a failed add so the user can retry", async () => {
    vi.mocked(window._popper_.addEudicWord!).mockRejectedValue(new Error("授权已过期"));
    render(<DictionaryPanel {...props()} />);
    fireEvent.click(screen.getByRole("button", { name: "加入欧路生词本" }));
  ...
  it("does not show old add results after selecting a different word", async () => {
    ...
    vi.mocked(window._popper_.addEudicWord!).mockReturnValue(pending.promise);
    const p = props();
    const { rerender } = render(<DictionaryPanel {...p} />);
    fireEvent.click(screen.getByRole("button", { name: "加入欧路生词本" }));
    fireEvent.change(await screen.findByRole("combobox"), {
      target: { value: "123456789012345678" },
    });
  ...
  it("keeps lookup errors separate from a missing entry", () => {
    render(
      <DictionaryPanel
        {...props()}
        snapshot={{ ...snapshot, status: "error", entry: null, error: "网络请求超时" }}
      />,
    );
  ...
  it("ignores old snapshot hydration and events for other windows", async () => {
    const initial = deferred<DictionarySnapshot | null>();
    let listener!: (s: DictionarySnapshot) => void;
    window._popper_.getDictionaryState = vi.fn().mockReturnValue(initial.promise);
    window._popper_.onDictionaryChanged = (fn) => {
      listener = fn;
      return () => {};
    };
