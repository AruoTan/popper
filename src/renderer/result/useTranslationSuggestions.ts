...
export function useTranslationSuggestions(sessionId: string, requestId: string | null, input: string, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    setItems([]); setError('')
    const suggest = window.textLens?.translationInputSuggestions
    if (!requestId || !suggest || !enabled) return
    void suggest(sessionId, requestId, ticket, input).then((next) => {
      if (version.current === ticket) setItems(next)
    }).catch((cause: unknown) => {
      if (version.current === ticket && enabled && input.trim()) setError(getErrorMessage(cause, '联想查询失败'))
    })
    return () => {
