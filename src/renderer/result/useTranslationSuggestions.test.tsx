...
import type { DictionarySuggestion, WindowTextLensApi } from '../../shared'
...
describe('translation input suggestions', () => {
  it('ignores old responses and cancels input on change, generation change and unmount', async () => {
    ...
    const old = new Promise<DictionarySuggestion[]>((resolve) => { finishOld = resolve })
    const suggest = vi.fn().mockImplementation((_id, _request, _version, text) =>
      text === 'ca' ? old : Promise.resolve(text ? [{ word: 'cat', explanation: '猫' }] : []))
    window.textLens = { translationInputSuggestions: suggest } as unknown as WindowTextLensApi
    const { result, rerender, unmount } = renderHook(({ input, request }) => useTranslationSuggestions('one', request, input, true),
  ...
  it('delegates recognition to Rust and cancels suggestions when disabled', async () => {
    ...
    window.textLens = { translationInputSuggestions: suggest } as unknown as WindowTextLensApi
    const { result, rerender } = renderHook(({ enabled }) => useTranslationSuggestions('one', 'request', '你好 world', enabled),
      { initialProps: { enabled: true } })
    await waitFor(() => expect(suggest).toHaveBeenLastCalledWith('one', 'request', expect.any(Number), '你好 world'))
    rerender({ enabled: false })
    await waitFor(() => expect(suggest).toHaveBeenLastCalledWith('one', 'request', expect.any(Number), ''))
