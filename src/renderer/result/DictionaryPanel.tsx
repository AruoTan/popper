...
export function DictionaryPanel({ snapshot, onAi, onQuery, suggestions = snapshot.suggestions, querying = false }: {
  ...
  useEffect(() => {
    setShowBooks(false); setCategory(''); setAdded([]); setMessage('')
    audio.current?.pause(); setAudioLoading(false)
  }, [snapshot.query, snapshot.queryGeneration])
  useEffect(() => () => { generation.current = -1; audio.current?.pause() }, [])

  const query = (word: string): void => { void onQuery(word) }
