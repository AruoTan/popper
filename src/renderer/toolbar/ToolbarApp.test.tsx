...
describe("ToolbarApp", () => {
  ...
  it("checks the current selection and retries when native presentation returns false", async () => {
    const presentToolbar = vi.fn().mockResolvedValueOnce(false).mockResolvedValueOnce(true);
    const recoverToolbar = vi.fn().mockResolvedValue(true);
    vi.spyOn(HTMLElement.prototype, "getBoundingClientRect").mockReturnValue(
      new DOMRect(0, 0, 212, 38),
    );
    const api = {
      getSettings: vi.fn().mockResolvedValue(DEFAULT_PUBLIC_SETTINGS),
      getCurrentSelection: vi.fn().mockResolvedValue(selection),
  ...
  it("keeps Ask first and expands it into an inline composer before starting the session", async () => {
    vi.spyOn(HTMLElement.prototype, "getBoundingClientRect").mockImplementation(function (
      ...
      const expanded =
        this.classList.contains("toolbar-shell") &&
    ...
    const setToolbarInputMode = vi
      .fn()
      .mockImplementationOnce(() => activation.promise)
      .mockResolvedValue(true);
    const focusToolbarInput = vi.fn().mockResolvedValueOnce(false).mockResolvedValue(true);
    const hideToolbar = vi.fn().mockResolvedValue(undefined);
    ...
    await waitFor(() => {
      expect(reportToolbarSize).toHaveBeenCalledWith({ width: 446, height: 100 }, "selection-1");
      ...
      expect(expandedSizeOrder).toBeLessThan(setToolbarInputMode.mock.invocationCallOrder[0]!);
  ...
  it("uses native client coordinates to move hover without a pressed mouse button", async () => {
    let pointerListener: ((pointer: { x: number; y: number; inside: boolean }) => void) | undefined;
