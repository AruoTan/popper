...
const action = (kind: ActionKind): ActionDefinition => {
  ...
  return {
    ...
    prompt: kind === "custom" ? "原文：{{text}}\n再写一次：{{text}}" : DEFAULT_ACTION_PROMPTS[kind],
  };
};

describe("prompt builder", () => {
  it("exposes the editable built-in defaults while keeping the Popper custom default", () => {
...
{{text}}
  ...
  it("preserves every placeholder found in selected source text", () => {
    ...
    for (const kind of ["translate", "summary", "explain"] as const) {
      ...
      expect(result.sourceBoundary?.begin).toMatch(/^<<<POPPER_SOURCE_/u);
  ...
  it("uses editable built-in prompts and expands the Cherry output-language placeholder", () => {
    ...
    expect(summaryResult.userPrompt).not.toContain(OUTPUT_LANGUAGE_PLACEHOLDER);
