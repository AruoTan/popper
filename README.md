...
- 动作可启停、改名、排序、换图标；支持仅显示图标；搜索可选用 Google / Bing / 百度，URL/域名/IP 优先直达网页。
- 多服务商、多模型（OpenAI-compatible）；每个 AI 动作可单独选模型与提示词；模型可获取后多选合并并排序。
- 「问AI」以划词为上下文打开结果窗，支持底部多轮继续提问。
- 结果窗流式输出，支持 Markdown、表格与公式；可展示模型 thinking；标题栏可切换模型并重新生成。
- 结果窗可拖动、缩放、置顶与多种关闭方式；正文可再次划词使用工具栏。
- 微信、WPS、飞书等自绘/兼容场景可回退剪贴板取词，并尽量恢复原剪贴板。
...
macOS 0.3.16 到 0.3.53 的完整开发复盘见 [DEVELOPMENT_HISTORY.md](./DEVELOPMENT_HISTORY.md)。
