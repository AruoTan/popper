...
TextLens 是一个轻量的桌面划词助手。当前源码版本是 **0.3.54**，面向 **macOS 12+ Apple Silicon（M 系列芯片）**和 **Windows 10/11 x64**，使用 Tauri 2、Rust、React 和 TypeScript 构建。
...
当前源码和文档已同步到 0.3.54。本版本加固 WPS 等复杂应用的划词工具栏捕获（Windows UIA 多进程关联与 allowlist 剪贴板回退、macOS 更宽的 WPS/金山 bundle 识别）；优化翻译结果 Markdown 的段落与列表换行（句末单换行、中文列表标记 `1、`/`•` 结构化，表格与代码块保持紧凑）；并修正思考区字号（去掉与正文同级的 `stream-plain-text` 覆盖，思考正文约为结果字号的 0.82 倍）。0.3.53 修复结果关闭后重划工具栏与结果内再划词 dismiss，并改进服务商模型获取与排序。Windows 测试安装包继续由 NSIS 构建。两个平台共用 renderer、设置、动作、模型请求、流式输出和数据契约，仅选区捕获、剪贴板、窗口原生属性和系统集成使用平台实现。
...
- 多服务商、多模型（OpenAI-compatible）；动作可单独绑定模型与提示词；支持获取模型后多选合并与排序。
...
- **标题栏换模重生：** 按服务商分组切换模型，并在同一结果框内重新生成；翻译还可切换目标语言。
- **问AI 会话：** 以划词为上下文打开结果窗，可先不请求模型，再在底部多轮提问；会话仅限当前结果窗。
- **思考与流式：** 支持 thinking / reasoning 流式展示；AI 动作可配置思考档位；流式输出跟手显示。
...
例如当前版本会导出为：
    ...
    portable-dev-sources/TextLens-0.3.54-dev-source

也可将上述目录复制到仓库外，作为独立的 `TextLens-0.3.54-dev-source` 文件夹或压缩包分发；包内只含源码与文档，便于整夹拷贝到其他机器继续开发。
...
1. 把整个 `TextLens-0.3.54-dev-source` 文件夹（或已重命名的 `TextLens` 工作区）复制到目标机器。
...
macOS 0.3.16 到 0.3.54 的完整开发复盘见 [DEVELOPMENT_HISTORY.md](./DEVELOPMENT_HISTORY.md)。
